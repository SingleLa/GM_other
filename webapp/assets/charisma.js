$(document).ready(function(){
	...
	{
		$('#bs-css').attr('href','/admin_css/bootstrap-'+theme_name+'.css');
	}
	...
	//$('.accordion > a').siblings('ul').slideUp();
	//$('.accordion li.active:first').parents('ul').hide();
	$('.accordion li.active:first').parents('ul').removeAttr("style");
...
function docReady(){
	...
	//datatable  api 文档介绍 http://blog.csdn.net/mickey_miki/article/details/8240477
	$('.datatable').dataTable({
		...
		//保存状态 使用了翻页或者改变了每页显示数据数量，会保存在cookie中，下回访问时会显示上一次关闭页面时的内容
		"bStateSave": true,
		} );
